    ...
    return new Promise(async (resolve, reject) => {
        ...
        chrome.runtime.sendMessage({ action: "save_owl", owlID: owlID, bbcode: bbcode, htmlContent: htmlContent }, (response) => {
            if (response.error || chrome.runtime.lastError) {
                reject("Error for owl " + owlID + ": " + (response.error || chrome.runtime.lastError));
            } else {
...
async function checkOldestOwl() {
    ...
    chrome.storage.sync.get({ owleryCapacityQuota: 95 }, function (items) {
                ...
                .then(html => {
                    ...
                    if (href) {
                        ...
                        saveOwl(owlID).then(response => {
                            ...
                            chrome.storage.sync.get({ deleteOldestOwl: false }, function (items) {
                                if(items.deleteOldestOwl) {
                                    chrome.runtime.sendMessage({ action: "delete_real_owl", owlID: owlID }, (response) => {
                                        if (chrome.runtime.lastError) {
                                    ...
                                    console.log("Owl saved but preferences are set to not delete it.");
                                }
                            });
                        }).catch(error => {
                            console.log(error);
...
chrome.storage.sync.get({ saveOldestOwl: true }, function (items) {
    ...
    if (table.children().length >= owlIndex + 1) {
        const $newRow = $(`
            <tr class="dlight" id="HPlusOwl_${owl.id || 0}">
                ...
                <td>
                    <a href="${chrome.runtime.getURL('savedOwls/savedOwl.html')}?owlID=${owl.id || 0}">${owl.title || "כותרת ינשוף"}</a>
                </td>
                <td>
        ...
        $("#DeleteHPlusOwl_" + owl.id).click(function () {
            chrome.runtime.sendMessage({ action: "delete_owl_from_db", owlID: owl.id }, (response) => {
                if (chrome.runtime.lastError) {
    ...
    try {
        chrome.runtime.sendMessage({ action: "get_all_owls" }, async (response) => {
            if (chrome.runtime.lastError) {
                ...
                let displayDuplicateOwls = await chrome.storage.sync.get('displayDuplicateOwls');
...
chrome.storage.sync.get({ owlsSinceExport: 0, owlsWarningInput: 5, displaySavedOwls: true }, function (items) {

...
let settingsButton = $("<div>", {
    ...
    css: {
        "background-image": "url('" + chrome.runtime.getURL("images/settings2.svg") + "')"
    }
});
...
chrome.storage.sync.get({ banMeMillisec: 0, lockBan: false }, function(data) {
    ...
    var checkTime = setInterval(() => {
        if ((new Date()).getTime() < data.banMeMillisec) {
            ...
            if(!data.lockBan) {
                rest = "את/ה יכול/ה לבטל את הבאן <a style='top: 50%; position: aboslute; color: white;' dir='rtl' href='" + chrome.runtime.getURL("options/options.html") + "' target='_blank'>בהגדרות התוסף</a>. ";
            } else {
                rest = "";
            }

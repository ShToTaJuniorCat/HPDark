...
document.addEventListener("DOMContentLoaded", async () => {
    let userSetQuota = await chrome.storage.sync.get('maxOwleryStorage'); // MB (user-set limit)
    userSetQuota = userSetQuota.maxOwleryStorage * 1048576; // Convert from MB to B
    let indexedDBUsed = await chrome.runtime.sendMessage({ action: "get_db_storage_size" });
    ...
    let savedOwlCount = await chrome.runtime.sendMessage({ action: "get_saved_owls_count" });
    ...
    let owlsSinceLastBackup = await chrome.storage.sync.get('owlsSinceExport'); // Owls added since last backup
